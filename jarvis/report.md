# JARVIS Crew — weekly inspection 2026-07-12

Pages audited: **43** · findings: **7** (0 high / 6 med / 1 low)
Scout intel: merged (snapshot 2026-07-12)

## MED (6)
- `index.html` **content-depth** → _content_ — ~804 words vs field avg ~1370 — deepen with scope details, FAQs, local specifics
- `student-turnover-cleaning.html` **content-depth** → _content_ — ~693 words vs field avg ~1370 — deepen with scope details, FAQs, local specifics
- `commercial-cleaning-albany-ny.html` **content-depth** → _content_ — ~837 words vs field avg ~1370 — deepen with scope details, FAQs, local specifics
- `post-construction-cleaning.html` **content-depth** → _content_ — ~576 words vs field avg ~1370 — deepen with scope details, FAQs, local specifics
- `property-management.html` **content-depth** → _content_ — ~401 words vs field avg ~1370 — deepen with scope details, FAQs, local specifics
- `renovations-construction.html` **content-depth** → _content_ — ~348 words vs field avg ~1370 — deepen with scope details, FAQs, local specifics

## LOW (1)
- `index.html` **img-alt** → _human_ — 1 <img> without alt

**Owners:** `fixer` = seo-autopilot handles on its next 6-hour run · `content`/`human` = needs Miguel or a drafted PR.

_Inspector is read-only; it routes work but never edits pages._